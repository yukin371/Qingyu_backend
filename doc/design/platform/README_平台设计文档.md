...
│  Gin HTTP Server                                        │
│  ├── Router (路由管理)                                 │
│  └── Middleware (认证/权限/限流/日志/CORS)              │
├─────────────────────────────────────────────────────────┤
│  业务模块层 (模块化设计，统一进程)                      │
│  ├── User Module (注册/登录/权限管理)                   │
│  ├── Work Module (CRUD/版本管理/搜索)                   │
│  ├── File Module (上传/下载/存储)                       │
│  └── Notification Module (消息推送/邮件通知)            │
├─────────────────────────────────────────────────────────┤
...
**架构说明**：
- 所有模块在同一进程内运行，通过 Go 接口直接调用
- Gin Router 直接处理路由，无需独立网关
- Redis Streams 用于轻量级异步任务处理
- Elasticsearch 为可选组件（数据量大时再引入）
