...
### 1.2 业务价值
- **系统解耦**：降低服务间的直接依赖，提高系统的灵活性和可维护性
- **异步处理**：提升系统响应速度，改善用户体验
- **流量削峰**：缓解高并发场景下的系统压力
- **数据一致性**：通过事务消息保证分布式事务的最终一致性
...
├─────────────────────────────────────────────────────────┤
│                   消息处理层                            │
│  消费者管理 + 重试机制 + 死信处理 + 事务支持             │
├─────────────────────────────────────────────────────────┤
│                    存储管理层                            │
│  消息持久化 + 元数据管理 + 监控数据 + 配置管理           │
└─────────────────────────────────────────────────────────┘
```

### 2.2 模块划分
- **消息生产者模块**：消息发送、序列化、路由选择
- **消息代理模块**：消息存储、分发、持久化管理
- **消息消费者模块**：消息接收、反序列化、业务处理
- **管理控制模块**：集群管理、监控告警、配置管理
