...
import (
	...
	interfaces "Qingyu_backend/repository/interfaces/shared"
	"context"
	"errors"
	"time"
...
// Create 创建公告
func (r *MongoAnnouncementRepository) Create(ctx context.Context, announcement *shared.Announcement) error {
	if announcement == nil {
...
// GetByID 根据ID获取公告
func (r *MongoAnnouncementRepository) GetByID(ctx context.Context, id primitive.ObjectID) (*shared.Announcement, error) {
	var announcement shared.Announcement
...
// List 根据过滤条件列出公告
func (r *MongoAnnouncementRepository) List(ctx context.Context, filter infra.Filter) ([]*shared.Announcement, error) {
	var query bson.M
	if filter != nil {
		query = bson.M(filter.GetConditions())
	} else {
	...
	if filter != nil {
		...
		if announcementFilter, ok := filter.(*shared.AnnouncementFilter); ok {
	...
	var announcements []*shared.Announcement
...
// GetActive 获取激活的公告列表
func (r *MongoAnnouncementRepository) GetActive(ctx context.Context, targetUsers string, limit, offset int) ([]*shared.Announcement, error) {
	now := time.Now()
	query := bson.M{
		"is_active": true,
		"$or": []bson.M{
...
func (r *MongoAnnouncementRepository) GetByType(ctx context.Context, announcementType string, limit, offset int) ([]*shared.Announcement, error) {
	opts := options.Find().
		SetLimit(int64(limit)).
		SetSkip(int64(offset)).
		SetSort(bson.D{{Key: "priority", Value: -1}, {Key: "created_at", Value: -1}})
...
func (r *MongoAnnouncementRepository) GetByTimeRange(ctx context.Context, startTime, endTime *time.Time, limit, offset int) ([]*shared.Announcement, error) {
...
func (r *MongoAnnouncementRepository) GetEffective(ctx context.Context, targetUsers string, limit int) ([]*shared.Announcement, error) {
	now := time.Now()
	query := bson.M{
		"is_active": true,
		"$or": []bson.M{
