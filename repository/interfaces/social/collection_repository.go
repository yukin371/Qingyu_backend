...
type CollectionRepository interface {
	...
	// Create 创建收藏
	Create(ctx context.Context, collection *social.Collection) error

	// GetByID 根据ID获取收藏
	GetByID(ctx context.Context, id string) (*social.Collection, error)
	...
	GetCollectionsByTag(ctx context.Context, userID string, tag string, page, size int) ([]*social.Collection, int64, error)
	...
	// CreateFolder 创建收藏夹
	CreateFolder(ctx context.Context, folder *social.CollectionFolder) error

	// GetFolderByID 根据ID获取收藏夹
	GetFolderByID(ctx context.Context, id string) (*social.CollectionFolder, error)

	// GetFoldersByUser 获取用户的收藏夹列表
	GetFoldersByUser(ctx context.Context, userID string) ([]*social.CollectionFolder, error)
	...
	GetPublicCollections(ctx context.Context, page, size int) ([]*social.Collection, int64, error)

	// GetPublicFolders 获取公开收藏夹列表
	GetPublicFolders(ctx context.Context, page, size int) ([]*social.CollectionFolder, int64, error)
