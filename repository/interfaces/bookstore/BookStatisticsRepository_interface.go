...
type BookStatisticsRepository interface {
	// 继承基础CRUD接口
	base.CRUDRepository[*bookstore.BookStatistics, string]
	// 继承 HealthRepository 接口
	base.HealthRepository

	// 统计特定查询方法
	GetByBookID(ctx context.Context, bookID string) (*bookstore.BookStatistics, error)
	...
	// 批量统计操作
	BatchIncrementViewCount(ctx context.Context, increments map[string]int64) error
	...
	BatchRecalculateStatistics(ctx context.Context, bookIDs []string) error
	...
	// 时间范围统计
	GetViewsInRange(ctx context.Context, bookID string, days int) (int64, error)
	GetFavoritesInRange(ctx context.Context, bookID string, days int) (int64, error)
	GetCommentsInRange(ctx context.Context, bookID string, days int) (int64, error)
