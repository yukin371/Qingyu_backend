...
import (
	...
	repo "Qingyu_backend/repository/interfaces/shared"
	"context"
	"time"
...
type AnnouncementService interface {
	// 基础CRUD
	GetAnnouncementByID(ctx context.Context, id string) (*shared.Announcement, error)
	GetAnnouncements(ctx context.Context, req *GetAnnouncementsRequest) (*GetAnnouncementsResponse, error)
	GetEffectiveAnnouncements(ctx context.Context, targetUsers string, limit int) ([]*shared.Announcement, error)
	CreateAnnouncement(ctx context.Context, req *CreateAnnouncementRequest) (*shared.Announcement, error)
...
type GetAnnouncementsResponse struct {
	Announcements []*shared.Announcement `json:"announcements"`
...
type CreateAnnouncementRequest struct {
	...
	TargetUsers string     `json:"targetUsers" validate:"required,oneof=all reader writer admin"`
...
type UpdateAnnouncementRequest struct {
	...
	StartTime   *time.Time `json:"startTime"`
	EndTime     *time.Time `json:"endTime"`
	TargetUsers *string    `json:"targetUsers" validate:"omitempty,oneof=all reader writer admin"`
}
...
func (s *announcementServiceImpl) GetAnnouncementByID(ctx context.Context, id string) (*shared.Announcement, error) {
...
func (s *announcementServiceImpl) GetAnnouncements(ctx context.Context, req *GetAnnouncementsRequest) (*GetAnnouncementsResponse, error) {
	...
	// 构建Filter
	filter := &shared.AnnouncementFilter{
...
func (s *announcementServiceImpl) GetEffectiveAnnouncements(ctx context.Context, targetUsers string, limit int) ([]*shared.Announcement, error) {
	if limit <= 0 {
		limit = 10
	}
	if limit > 50 {
...
func (s *announcementServiceImpl) CreateAnnouncement(ctx context.Context, req *CreateAnnouncementRequest) (*shared.Announcement, error) {
	// 验证时间范围
	...
	// 设置创建者
	if req.CreatedBy != "" {
		createdByID, err := primitive.ObjectIDFromHex(req.CreatedBy)
		if err == nil {
			announcement.CreatedBy = createdByID
		}
	}
...
func (s *announcementServiceImpl) UpdateAnnouncement(ctx context.Context, id string, req *UpdateAnnouncementRequest) error {
	...
	if req.TargetUsers != nil {
		updates["target_users"] = *req.TargetUsers
	}
