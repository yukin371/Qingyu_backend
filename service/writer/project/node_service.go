...
func NewNodeService(db *mongo.Database) *NodeService {
	return &NodeService{db: db}
}

func (s *NodeService) nodeCol() *mongo.Collection { return s.db.Collection("nodes") }
...
func (s *NodeService) RenameNode(projectID, nodeID, newName string) error {
	...
	var n model.Node
	if err := s.nodeCol().FindOne(ctx, bson.M{"_id": nodeID, "project_id": projectID}).Decode(&n); err != nil {
		return err
	}
	...
	if n.ParentID != "" {
		...
		if err := s.nodeCol().FindOne(ctx, bson.M{"_id": n.ParentID, "project_id": projectID}).Decode(&parent); err != nil {
			return err
		}
	...
	if _, err := s.nodeCol().UpdateOne(ctx, bson.M{"_id": nodeID, "project_id": projectID}, update); err != nil {
		return err
	}

	// 级联更新子孙节点：查出以 oldPath 为前缀的所有节点
	cursor, err := s.nodeCol().Find(ctx, bson.M{
	...
	for cursor.Next(ctx) {
		...
		if _, err := s.nodeCol().UpdateOne(ctx, bson.M{"_id": child.ID}, bson.M{"$set": bson.M{"relative_path": rel, "updated_at": time.Now()}}); err != nil {
			return err
		}
	}
...
func (s *NodeService) MoveNode(projectID, nodeID, newParentID string) error {
	...
	if newParentID != "" {
		var parent model.Node
		if err := s.nodeCol().FindOne(ctx, bson.M{"_id": newParentID, "project_id": projectID}).Decode(&parent); err != nil {
			return err
		}
		parentPath = parent.RelativePath
	}
	...
	if _, err := s.nodeCol().UpdateOne(ctx, bson.M{"_id": nodeID}, bson.M{"$set": bson.M{
