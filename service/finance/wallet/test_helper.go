...
func (m *MockWalletRepositoryV2) GetWallet(ctx context.Context, userID string) (*financeModel.Wallet, error) {
...
// GetTransaction 获取交易
func (m *MockWalletRepositoryV2) GetTransaction(ctx context.Context, transactionID string) (*financeModel.Transaction, error) {
...
func (m *MockWalletRepositoryV2) ListTransactions(ctx context.Context, filter *sharedRepo.TransactionFilter) ([]*financeModel.Transaction, error) {
	...
	if filter.Limit > 0 {
		...
		if start > len(result) {
			return []*financeModel.Transaction{}, nil
		}
		if end > len(result) {
			end = len(result)
		}
...
func (m *MockWalletRepositoryV2) GetWithdrawRequest(ctx context.Context, withdrawID string) (*financeModel.WithdrawRequest, error) {
