...
func ToUserDTO(user *usersModel.User) *UserDTO {
	if user == nil {
		return nil
	}
...
func ToUserDTOs(users []usersModel.User) []*UserDTO {
...
// ToUserModel 从 DTO 创建 Model（用于更新）
// 注意：不包含密码字段，密码更新应该使用专门的方法
func ToUserModel(dto *UserDTO) (*usersModel.User, error) {
	if dto == nil {
		return nil, nil
	}
...
// ToUserModelWithoutID 从 DTO 创建 Model（用于创建新用户）
// 不设置 ID，让数据库自动生成
func ToUserModelWithoutID(dto *UserDTO) (*usersModel.User, error) {
	if dto == nil {
		return nil, nil
	}
