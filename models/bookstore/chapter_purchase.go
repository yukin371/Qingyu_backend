...
type ChapterPurchaseBatch struct {
	...
	UserID        primitive.ObjectID   `bson:"user_id" json:"user_id"`
	BookID        primitive.ObjectID   `bson:"book_id" json:"book_id"`
	ChapterIDs    []primitive.ObjectID `bson:"chapter_ids" json:"chapter_i_ds"`
	TotalPrice    float64             `bson:"total_price" json:"total_price"` // 总价 (分，使用float64以兼容MongoDB)
...
type BookPurchase struct {
	...
	BookID        primitive.ObjectID `bson:"book_id" json:"book_id"`
	TotalPrice    float64            `bson:"total_price" json:"total_price"`       // 总价 (分，使用float64以兼容MongoDB)
	OriginalPrice float64            `bson:"original_price" json:"original_price"` // 原价 (分，使用float64以兼容MongoDB)
...
type ChapterAccessInfo struct {
	...
	Price        float64            `json:"price"` // 价格 (分，使用float64以兼容MongoDB)
