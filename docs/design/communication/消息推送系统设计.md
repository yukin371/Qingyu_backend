...
本文档是消息推送能力的历史详细设计稿，适合回看 Push / 邮件 / 短信在模块化单体时期的分层、渠道与运维规划；它不等同于当前通信链路或通知实现的 owner。

## Page Role

- 这里负责：消息推送系统的历史需求、架构、分层设计、数据模型、接口与运维方案。
- 不负责：当前推送能力的实际代码入口、现行接口事实、当前实施状态结论。
...
- 如果你要找“当前通信能力的目录入口”，优先看 [README.md](./README.md)。
- 如果你要找“现行架构与模块边界”，优先看 [../../architecture/README.md](../../architecture/README.md)。
- 如果你要找“已经落地的实现或阶段记录”，优先看 [../../implementation/README.md](../../implementation/README.md)。
- 如果你要找“当前对外接口 owner”，优先看 [../../api/README.md](../../api/README.md)。

## Quick Section Map

| 如果你想看 | 直接跳到 |
|------|------|
| 需求边界和业务目标 | [1. 需求概述](#1-需求概述) |
| 整体架构、模块拆分、数据流与技术选型 | [2. 架构设计](#2-架构设计) |
| Router / API / Service / Repository / Model 详细设计 | [3.1 Router层设计](#31-router层设计) / [3.2 API层设计](#32-api层设计) / [3.3 Service层设计](#33-service层设计) / [3.4 Repository层设计](#34-repository层设计) / [3.5 Model层设计](#35-model层设计) |
| 数据模型、缓存与索引策略 | [4. 数据设计](#4-数据设计) |
| RESTful API 与 WebHook | [5. 接口设计](#5-接口设计) |
| 安全、测试、部署运维 | [6. 安全设计](#6-安全设计) / [7. 测试设计](#7-测试设计) / [8. 部署和运维](#8-部署和运维) |
| 风险与实施计划 | [9. 风险评估](#9-风险评估) / [10. 实施计划](#10-实施计划) |

## Quick Takeaways

- 这篇最值得先看的，是它把消息推送拆成“渠道管理、模板系统、用户偏好、统计分析、外部供应商接入”几个维度，而不只是单一发消息流程。
- 如果你只想理解系统边界，通常先看“2. 架构设计”“3. 详细设计”“5. 接口设计”就够了。
- 安全、测试、部署和实施计划更偏阶段方案背景，阅读时不应直接把它们当成当前落地事实。

## Skip Guide

- 只想知道“消息推送系统是怎么分层的”：看 [2. 架构设计](#2-架构设计) 和 [3. 详细设计](#3-详细设计)。
- 只想知道“对外暴露哪些能力”：看 [5. 接口设计](#5-接口设计)。
- 只想知道“数据怎么落库、缓存怎么组织”：看 [4. 数据设计](#4-数据设计)。
...
**未来升级条件**：
- 日推送量 > 10万时，考虑RabbitMQ
- 需要复杂监控分析时，考虑Prometheus + Grafana
