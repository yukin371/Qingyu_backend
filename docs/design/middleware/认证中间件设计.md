...
本文档是认证中间件的历史详细设计稿，适合回看 JWT 校验、用户上下文、权限检查和会话续期当时如何规划；它不直接等于当前接入 owner。

## Page Role

- 这里负责：认证中间件的架构、接口、安全、测试与部署设想。
- 不负责：当前接入步骤、当前分层规范、当前路由真实挂载事实。
...
| 如果你想看 | 直接跳到 |
|------|------|
| 认证中间件整体架构 | [2. 架构设计](#2-架构设计) |
| Router / API / Service / Model 如何分工 | [3. 详细设计](#3-详细设计) |
| 数据模型与索引 | [4. 数据设计](#4-数据设计) |
| 中间件接口与错误处理 | [5. 接口设计](#5-接口设计) |
| 权限、安全、审计 | [6. 安全设计](#6-安全设计) |
| 测试、运维、风险、实施计划 | [7. 测试设计](#7-测试设计) / [8. 部署和运维](#8-部署和运维) / [9. 风险评估](#9-风险评估) / [10. 实施计划](#10-实施计划) |

## Quick Takeaways

- 这篇最值得先看的，是认证链路如何拆成“令牌验证 + 权限检查 + 用户上下文 + 会话续期”。
- 如果你只想理解设计边界，读完“架构设计”“详细设计”“接口设计”通常就够了。
- 部署、风险和实施计划更偏阶段性背景，第一次可后看。

## Skip Guide

- 只想知道“认证中间件怎么走”：看 [2. 架构设计](#2-架构设计) 和 [3. 详细设计](#3-详细设计)。
- 只想知道“安全和权限怎么控”：看 [5. 接口设计](#5-接口设计) 与 [6. 安全设计](#6-安全设计)。
- 如果你当前只做现有链路接入，不必细读整篇；优先回到 `docs/middleware/README.md` 与 `docs/architecture/README.md`。
