...
本文档是日志中间件的历史详细设计稿，适合回看请求/响应日志、性能指标和审计字段当时如何设计；它不直接等于当前日志 owner。

## Page Role

- 这里负责：日志中间件的架构、字段标准、接口、安全、测试与运维设想。
- 不负责：当前 logger 实现 owner、当前日志落盘策略、当前监控事实。
...
| 如果你想看 | 直接跳到 |
|------|------|
| 日志中间件整体架构 | [2. 架构设计](#2-架构设计) |
| Router / API / Service / Model 设计 | [3. 详细设计](#3-详细设计) |
| 日志配置、字段与级别 | [4. 数据设计](#4-数据设计) |
| 中间件接口与响应写入器 | [5. 接口设计](#5-接口设计) |
| 敏感信息保护与安全 | [6. 安全设计](#6-安全设计) |
| 测试、运维、风险、实施计划 | [7. 测试设计](#7-测试设计) / [8. 部署和运维](#8-部署和运维) / [9. 风险评估](#9-风险评估) / [10. 实施计划](#10-实施计划) |

## Quick Takeaways

- 这篇最值得先看的，是日志字段标准、响应写入器和性能/安全之间的平衡思路。
- 如果你只想知道日志中间件怎么挂、记录什么、怎么保护敏感信息，读“架构设计”“数据设计”“接口设计”“安全设计”即可。
- 运维、风险和实施阶段更像补充背景。

## Skip Guide

- 只想知道“日志中间件核心结构”：看 [2. 架构设计](#2-架构设计) 和 [5. 接口设计](#5-接口设计)。
- 只想知道“日志里该放什么、哪些要脱敏”：看 [4. 数据设计](#4-数据设计) 和 [6. 安全设计](#6-安全设计)。
- 如果你当前只看运行态日志链路，不必细读整篇；优先回到 `docs/architecture/README.md` 与 `docs/ops/README.md`。
