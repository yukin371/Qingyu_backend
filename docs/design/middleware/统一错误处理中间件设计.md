...
本文档是统一错误处理中间件的历史详细设计稿，适合回看错误分类、响应构建、错误链管理和扩展机制当时如何规划；它不直接等于当前错误处理 owner。

## Page Role

- 这里负责：统一错误结构、错误处理中间件、错误响应格式、高级能力和扩展机制。
- 不负责：当前错误码标准 owner、当前 response helper 实现 owner。
...
| 如果你想看 | 直接跳到 |
|------|------|
| UnifiedError 结构和核心组件 | [架构设计](#架构设计) |
| 中间件本身怎么处理错误 | [中间件实现](#中间件实现) |
| Service / API / Middleware 怎么用 | [使用方法](#使用方法) |
| 标准错误响应长什么样 | [错误响应格式](#错误响应格式) |
| 错误链、聚合器、上下文包装 | [高级功能](#高级功能) |
| 最佳实践、扩展机制、监控测试 | [最佳实践](#最佳实践) / [扩展机制](#扩展机制) / [监控和告警](#监控和告警) / [测试策略](#测试策略) |

## Quick Takeaways

- 这篇最有价值的是把错误处理拆成“统一结构 + 中间件处理 + 响应格式 + 高级扩展”四层。
- 如果你只想理解错误系统的骨架，读“架构设计”“中间件实现”“错误响应格式”通常就够了。
- 高级功能和扩展机制更适合作为第二遍参考。

## Skip Guide

- 只想知道“统一错误系统核心长什么样”：看 [架构设计](#架构设计)。
- 只想知道“请求出错时中间件怎么回”：看 [中间件实现](#中间件实现) 与 [错误响应格式](#错误响应格式)。
- 如果你当前只在核对现行错误码和响应标准，不必通读本页；优先回到 `docs/standards/`。
