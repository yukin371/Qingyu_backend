...
本文档是权限中间件的历史详细设计稿，适合回看 RBAC、管理员权限、VIP 权限、配置管理和安全策略当时如何规划；它不直接等于当前接入 owner。

## Page Role

- 这里负责：权限中间件设计背景、层级模型、检查流程、配置管理、安全与性能策略。
- 不负责：当前接入步骤、当前分层规范、当前路由真实挂载情况。
...
| 如果你想看 | 直接跳到 |
|------|------|
| 权限模型和检查流程 | [2.1 权限层级模型](#21-权限层级模型) / [2.2 权限检查流程](#22-权限检查流程) |
| RBAC / 管理员 / VIP 三类中间件 | [3.1 RBAC权限中间件](#31-rbac权限中间件) / [3.2 管理员权限中间件](#32-管理员权限中间件) / [3.3 VIP会员权限中间件](#33-vip会员权限中间件) |
| 配置怎么组织 | [4. 权限配置管理](#4-权限配置管理) |
| 安全与审计策略 | [5. 安全设计](#5-安全设计) |
| 缓存、批量检查、预加载 | [6. 性能优化](#6-性能优化) |
| 测试、示例、监控与最佳实践 | [7. 测试设计](#7-测试设计) / [8. 使用示例](#8-使用示例) / [9. 监控与告警](#9-监控与告警) / [10. 最佳实践](#10-最佳实践) |

## Quick Takeaways

- 这篇真正值得优先看的，是权限层级模型、RBAC/管理员/VIP 三类中间件的职责拆分，以及配置管理的整体思路。
- 安全、防攻击、性能缓存这些章节更适合作为第二遍补充阅读，而不是第一次的主线。
- 如果你当前只关心“权限规则归谁管”，本页是历史设计稿，不应替代现行标准页。

## Skip Guide

- 只想知道“权限检查整体怎么走”：看 [2.1 权限层级模型](#21-权限层级模型) 和 [2.2 权限检查流程](#22-权限检查流程)。
- 只想知道“RBAC / 管理员 / VIP 分别怎么管”：看 [3. 详细设计](#3-详细设计)。
- 如果你只是做现有链路接入，不必细读安全和性能章节；优先回到 `docs/middleware/README.md` 与 `docs/standards/layer-middleware.md`。

## Boundary

- 如果你要找“现在中间件怎么接”，优先看 [../../middleware/README.md](../../middleware/README.md)。
- 如果你要找“权限中间件当前必须遵守什么规则”，优先看 [../../standards/layer-middleware.md](../../standards/layer-middleware.md)。
- 如果你要找“当前请求链和模块边界”，优先看 [../../architecture/README.md](../../architecture/README.md)。
...
- [共享服务设计归档入口](../shared/README.md) - RBAC、会话等共享服务旧设计已迁入归档区
