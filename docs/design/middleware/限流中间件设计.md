...
本文档是限流中间件的历史详细设计稿，适合回看令牌桶、多维度限流、Redis 分布式限流与 VIP 差异化策略当时如何规划；它不直接等于当前接入 owner。

## Page Role

- 这里负责：限流算法、核心结构、分布式策略、配置、监控和测试设想。
- 不负责：当前生产限流参数、当前真实阈值、当前运维策略。
...
| 如果你想看 | 直接跳到 |
|------|------|
| 限流整体架构和算法选择 | [2. 架构设计](#2-架构设计) |
| 令牌桶实现与中间件实现 | [3. 详细设计](#3-详细设计) |
| VIP / 动态 / 分级限流 | [4. 高级特性](#4-高级特性) |
| 配置和监控告警 | [5. 配置管理](#5-配置管理) / [6. 监控与告警](#6-监控与告警) |
| 性能、测试、示例与最佳实践 | [7. 性能优化](#7-性能优化) / [8. 测试设计](#8-测试设计) / [9. 使用示例](#9-使用示例) / [10. 最佳实践](#10-最佳实践) |

## Quick Takeaways

- 这篇最值得看的，是“令牌桶 + Redis 分布式限流 + VIP 差异化”三层组合。
- 如果你只想理解核心方案，读完“架构设计”“详细设计”“高级特性”即可。
- 使用示例和最佳实践更像补充层，第一次可后看。

## Skip Guide

- 只想知道“限流器本体怎么实现”：看 [3. 详细设计](#3-详细设计)。
- 只想知道“不同用户怎么差异化”：看 [4. 高级特性](#4-高级特性)。
- 如果你当前只关心现行限流阈值，不必细读本页；优先回到 `docs/ops/` 和当前配置 owner。
