...
- current-bounded: 历史代码重构迁移计划，只记录当时的阶段划分与风险控制
...
- 本页是历史迁移计划，不是当前执行 owner。
- 阶段、时间和回滚策略都带明显时间性。
- 当前活跃计划已迁到父仓 `docs/plans/submodules/backend/`。
...
- 这页记录的是历史代码重构迁移路线。
- 当前计划不应直接以它为 source of truth。

## Skip Guide

- 只看当前计划：跳过本文件。
- 只看旧迁移步骤：看迁移阶段规划和风险控制即可。
