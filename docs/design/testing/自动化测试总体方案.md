...
- 这里负责：自动化测试体系的历史完整方案，包括测试框架、CI/CD、覆盖率目标、数据管理与实施策略。
- 不负责：当前测试流水线事实、现行运行命令、当前覆盖率或完成度结论。
...
- 如果你要找“当前测试标准和分层规则”，优先看 [../../standards/testing/README.md](../../standards/testing/README.md)。
- 本页更适合回看历史自动化测试体系设计，不适合作为当前执行事实的唯一依据。

## Quick Section Map

| 如果你想看 | 直接跳到 |
|------|------|
| 目标、边界和业务价值 | [1. 需求概述](#1-需求概述) |
| 自动化测试整体架构与模块 | [2. 架构设计](#2-架构设计) |
| 测试框架、模式、助手库与 Mock 策略 | [3. 详细设计](#3-详细设计) |
| CI/CD 与本地命令 | [4. CI/CD集成](#4-cicd集成) |
| 覆盖率、质量门禁与测试数据 | [5. 测试覆盖率目标](#5-测试覆盖率目标) / [6. 测试数据管理](#6-测试数据管理) |
| 实施策略、监控报告、最佳实践与附录 | [7. 测试实施策略](#7-测试实施策略) / [8. 监控和报告](#8-监控和报告) / [9. 最佳实践](#9-最佳实践) / [11. 附录](#11-附录) |

## Quick Takeaways

- 这篇最值得先看的，是它把“测试框架、CI/CD、覆盖率、测试数据和实施阶段”放成一套完整方案，而不是零散 checklist。
- 如果你只关心自动化测试体系怎么搭，优先看“2. 架构设计”“3. 详细设计”“4. CI/CD集成”“5. 测试覆盖率目标”。
- 文中的覆盖率目标、时间指标和阶段计划是历史方案口径，不应直接视为当前承诺。

## Skip Guide

- 只想知道“自动化测试体系怎么分层”：看 [2. 架构设计](#2-架构设计)。
- 只想知道“框架和 Mock 怎么选”：看 [3. 详细设计](#3-详细设计)。
- 只想知道“CI/CD 和本地命令怎么组织”：看 [4. CI/CD集成](#4-cicd集成)。
- 如果你当前只关心现行运行方式或现网状态，不必细读本页；优先回到 `docs/testing/` 和 `docs/standards/testing/`。
