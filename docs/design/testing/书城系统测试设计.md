...
- 这里负责：书城模块的历史测试设计，包括分层测试、性能测试、安全测试、环境准备与执行计划。
- 不负责：当前书城测试的运行步骤、当前测试规范 owner、现行覆盖率或完成度结论。
...
- 如果你要找“当前怎么跑书城或阅读测试”，优先看 [../../testing/README_当前指南.md](../../testing/README_当前指南.md)。
- 如果你要找“当前测试标准与分层规则”，优先看 [../../standards/testing/README.md](../../standards/testing/README.md)。
- 本页更适合回看书城模块当时的测试覆盖思路，不适合作为当前执行事实的唯一依据。

## Quick Section Map

| 如果你想看 | 直接跳到 |
|------|------|
| 测试目标、范围和策略 | [1. 测试概述](#1-测试概述) |
| 单元 / 集成 / E2E 分类设计 | [2. 测试分类](#2-测试分类) |
| 功能、边界与异常用例 | [3. 测试用例设计](#3-测试用例设计) |
| 性能与安全测试 | [4. 性能测试](#4-性能测试) / [5. 安全测试](#5-安全测试) |
| 环境、执行、质量与风险 | [6. 测试环境](#6-测试环境) / [7. 测试执行](#7-测试执行) / [8. 测试质量保证](#8-测试质量保证) / [9. 风险评估](#9-风险评估) |

## Quick Takeaways

- 这篇最有价值的地方，是它把书城模块的分层测试、性能、安全和执行计划放成一份完整的模块级测试设计。
- 如果你只关心书城测试应覆盖哪些面，优先看“2. 测试分类”“3. 测试用例设计”“4. 性能测试”“5. 安全测试”。
- 文中的执行命令和计划属于历史方案口径，不应直接视为当前运行入口。

## Skip Guide

- 只想知道“书城测试要测哪些东西”：看 [1. 测试概述](#1-测试概述) 和 [2. 测试分类](#2-测试分类)。
- 只想知道“有哪些关键用例”：看 [3. 测试用例设计](#3-测试用例设计)。
- 只想知道“性能和安全怎么测”：看 [4. 性能测试](#4-性能测试) 和 [5. 安全测试](#5-安全测试)。
- 如果你当前只关心现行运行方式或规范，请优先回到 `docs/testing/` 和 `docs/standards/testing/`。
