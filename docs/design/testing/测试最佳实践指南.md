...
- 这里负责：Go 测试实践的历史经验总结，包括 Table-Driven、Mock、AAA、性能测试与反模式示例。
- 不负责：当前测试运行入口、现行标准的唯一口径、当前项目测试覆盖率事实。
...
- 如果你要找“当前测试怎么跑”，优先看 [../../testing/README.md](../../testing/README.md)。
- 如果你要找“当前测试规范 owner”，优先看 [../../standards/testing/README.md](../../standards/testing/README.md)。
- 本页更适合回看历史实践沉淀，不适合作为当前唯一规范依据。

## Quick Section Map

| 如果你想看 | 直接跳到 |
|------|------|
| 测试金字塔和层级重点 | [1. 测试金字塔原则](#1-测试金字塔原则) |
| Table-Driven 与 Mock 实践 | [2. Table-Driven测试模式](#2-table-driven测试模式) / [3. Mock使用最佳实践](#3-mock使用最佳实践) |
| 数据工厂、AAA 与命名规范 | [4. 测试数据工厂模式](#4-测试数据工厂模式) / [5. AAA测试模式](#5-aaa测试模式) / [6. 测试命名规范](#6-测试命名规范) |
| 常见场景与性能测试 | [7. 常见测试场景](#7-常见测试场景) / [8. 性能测试指南](#8-性能测试指南) |
| 审查清单、反模式与快速参考 | [9. 测试代码审查清单](#9-测试代码审查清单) / [10. 常见反模式](#10-常见反模式) / [11. 快速参考](#11-快速参考) |

## Quick Takeaways

- 这篇最有价值的地方，是它把测试模式、Mock 习惯、代码审查清单和反模式放在一份可跳读的实践手册里。
- 如果你只想快速提升测试写法，优先看“2. Table-Driven测试模式”“3. Mock使用最佳实践”“5. AAA测试模式”“10. 常见反模式”。
- 它属于历史实践沉淀，不应直接替代 `docs/standards/testing/` 的现行口径。

## Skip Guide

- 只想知道“测试怎么组织”：看 [1. 测试金字塔原则](#1-测试金字塔原则)。
- 只想知道“Go 测试代码怎么写得更稳”：看 [2. Table-Driven测试模式](#2-table-driven测试模式)、[3. Mock使用最佳实践](#3-mock使用最佳实践)、[5. AAA测试模式](#5-aaa测试模式)。
- 只想知道“有哪些常见坑”：看 [10. 常见反模式](#10-常见反模式)。
- 如果你当前只关心现行测试规范，请优先回到 `docs/testing/` 和 `docs/standards/testing/`。
