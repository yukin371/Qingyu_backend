...
- 这里负责：版本管理服务的历史重构方案，包括新模型、Repository 接口、Service 设计和迁移策略。
- 不负责：当前版本服务真实代码结构、现行接口事实、当前重构是否已完成的结论。
...
5. [../../../standards/README.md](../../../standards/README.md)

## Boundary

- 如果你要找“当前架构与项目边界”，优先看 [../../../architecture/README.md](../../../architecture/README.md)。
...
- 本页更适合解释版本服务当时为什么要重构，不适合作为当前实现事实的唯一依据。

## Quick Section Map

| 如果你想看 | 直接跳到 |
|------|------|
| 为什么需要重构 | [1. 为什么需要重构](#1-为什么需要重构) |
| 新架构与数据模型 | [2. 新架构设计](#2-新架构设计) / [3. 数据模型设计（新架构）](#3-数据模型设计新架构) |
| Repository / Service / 事件设计 | [4. Repository接口设计](#4-repository接口设计) / [5. Service层设计（新架构）](#5-service层设计新架构) / [6. 事件设计](#6-事件设计) |
| 迁移、性能与实现参考 | [7. 迁移策略](#7-迁移策略) / [8. 性能优化](#8-性能优化) / [9. 实现参考](#9-实现参考) |
| 测试与收尾 | [10. 测试策略](#10-测试策略) / [11. 总结](#11-总结) |

## Quick Takeaways

- 这篇最有价值的地方，是它把版本服务重构的原因、新模型和渐进式迁移策略放成了一份完整方案。
- 如果你只关心重构主线，优先看“1. 为什么需要重构”“2. 新架构设计”“4. Repository接口设计”“5. Service层设计（新架构）”“7. 迁移策略”。
- “需要重构”是历史方案判断，不应直接视为当前仓库现状。

## Skip Guide

- 只想知道“为什么旧实现不行”：看 [1. 为什么需要重构](#1-为什么需要重构)。
- 只想知道“新架构要怎么做”：看 [2. 新架构设计](#2-新架构设计) 到 [5. Service层设计（新架构）](#5-service层设计新架构)。
- 如果你当前只关心现行架构或实施状态，请优先回到 `docs/architecture/`、`docs/implementation/`。
