...
- 这里负责：审核系统数据模型的历史设计方案，包括审核记录、敏感词、违规记录与风险评分。
- 不负责：当前审核模型事实、现行数据库 owner、当前风险分数算法结论。

## Recommended Read Path

1. [README.md](./README.md)
2. [内容审核规则引擎设计.md](./内容审核规则引擎设计.md)
3. [../../../architecture/README.md](../../../architecture/README.md)
4. [../../../implementation/README.md](../../../implementation/README.md)

## Boundary

- 如果你要找“当前审核系统架构边界”，优先看 [../../../architecture/README.md](../../../architecture/README.md)。
- 如果你要找“已落地的审核实现记录”，优先看 [../../../implementation/README.md](../../../implementation/README.md)。
- 本页更适合解释审核系统数据模型当时如何设计，不适合作为当前实现事实的唯一依据。

## Quick Section Map

| 如果你想看 | 直接跳到 |
|------|------|
| 业务背景与实现情况 | 见“1. 为什么需要这个设计” |
| 各核心数据模型 | 见“2. 数据模型设计” |
| 数据关系与风险评分 | 见“3. 数据关系”“4. 风险评分算法” |
| 与整体架构关系 | 见“5. 与v2.1架构的关系” |
| 存储优化、实现参考、示例与扩展 | 见“6. 存储优化”到“9. 未来扩展” |

## Quick Takeaways

- 这篇最有价值的地方，是它把审核记录、敏感词、违规记录和风险评分放成了一份完整的数据模型设计。
- 如果你只关心审核系统怎么建模，优先看“2. 数据模型设计”“3. 数据关系”“4. 风险评分算法”。
- “已实现”属于历史文档口径，不应直接视为当前模型状态。

## Skip Guide

- 只想知道“审核系统要存哪些数据”：看“2. 数据模型设计”。
- 只想知道“风险怎么评估”：看“4. 风险评分算法”。
- 如果你当前只关心现行模型或落地状态，请优先回到 `docs/architecture/`、`docs/implementation/`。
