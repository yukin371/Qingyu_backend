...
本文档是数据库迁移体系的历史详细设计稿，适合回看迁移机制为什么这样设计、早期命令模型如何拆分；但它不是当前数据库操作文档的主入口。

## Page Role

- 这里负责：数据库迁移系统的详细设计背景、接口抽象与架构拆分。
- 不负责：当前数据库操作入口、当前迁移实践手册、当前文档分类规则。
...
- 如果你要找“现在数据库相关文档从哪看”，优先回到 [../../database/README.md](../../database/README.md)。
- 如果你要找“迁移说明与兼容退出边界”，优先看 [../../migration/README.md](../../migration/README.md)。
- 如果你要找“当前分层和长期规则”，优先看 [../../standards/README.md](../../standards/README.md) 与 [../../architecture/README.md](../../architecture/README.md)。

## Quick Section Map

| 如果你想看 | 直接跳到 |
|------|------|
| 迁移系统整体架构 | [2. 架构设计](#2-架构设计) |
| Migration 接口和管理器职责 | [2.2 Migration 接口设计](#22-migration-接口设计) / [3. 详细设计](#3-详细设计) |
| 版本控制和迁移记录 | [4. 版本管理](#4-版本管理) |
| 种子数据与初始化 | [5. 种子数据设计](#5-种子数据设计) |
| CLI 和使用流程 | [6. 命令行接口设计](#6-命令行接口设计) / [7. 使用流程](#7-使用流程) |
| 测试、监控与总结 | [8. 测试策略](#8-测试策略) / [9. 监控和告警](#9-监控和告警) / [10. 总结](#10-总结) |

## Quick Takeaways

- 这篇的核心价值在于把迁移系统拆成“命令行工具 + 管理器 + Migration 接口 + 版本记录 + Seeds”五层。
- 如果你只是要理解迁移体系为何存在、如何分责，读完“架构设计”“详细设计”“版本管理”就够了。
- CLI、测试、监控章节更像配套补充，第一次不一定要完整读完。

## Skip Guide

- 只想知道“迁移系统长什么样”：看 [2. 架构设计](#2-架构设计)。
- 只想知道“Manager / Migration 接口怎么分工”：看 [3. 详细设计](#3-详细设计)。
- 如果你当前只是跑数据库文档操作，不必细读整篇；优先回到 `docs/database/README.md` 与 `docs/migration/README.md`。
...
- [后端架构总览](../../architecture/README.md) - 当前架构入口
- [项目四层架构CRUD设计](./project/项目四层架构CRUD设计.md) - Schema设计
- [核心功能设计入口](./README.md) - 核心基础设施总入口
