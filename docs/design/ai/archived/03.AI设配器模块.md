...
- current-bounded: 旧版多厂商 AI 适配器模块设计，仅作历史参考

## Recommended Read Path

1. 先读 `../README.md`。
2. 再读 `../agent/08.Go_AI代理层设计.md`。
3. 只有需要对照旧适配器层拆分时，再看本文件。

## Boundary

- 本页描述的是被新代理层吸收的旧适配器方案。
- 当前适配器边界应以 Go AI 代理层设计为准。
- 不应在此继续追加 today 方案。
...
- 这是旧适配器模块文档。
- 当前 owner 已迁入 `08.Go_AI代理层设计.md`。

## Skip Guide

- 只看当前代理层：跳过本文件。
- 只看旧适配器拆分思路：看概述和核心组件即可。
