...
- current-bounded: 旧版上下文管理方案，现已被 RAG 检索增强和新主线吸收替代

## Recommended Read Path

1. 先读 `../README.md`。
2. 再读 `../rag/10.RAG检索增强系统设计.md`。
3. 需要对照旧版记忆/上下文思路时，再读本文件。

## Boundary

- 本页是旧上下文管理设计，不再承担当前上下文 owner。
- 当前上下文相关能力已迁到 RAG 与 Agent 主线。
- 不适合作为 today 长文本上下文设计依据。
...
- 这是旧版上下文管理设计。
- 当前相关 owner 已转到 `rag/10` 和主线 Agent 方案。

## Skip Guide

- 只看当前上下文方案：跳过本文件。
- 只看旧上下文思路：看系统架构和上下文模型即可。
