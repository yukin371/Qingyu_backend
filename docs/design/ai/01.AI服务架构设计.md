...
- current-bounded: 根层保留的旧架构副本，只用于就近提醒读者该方案已被替代

## Recommended Read Path

1. 先读 `README.md`。
2. 再读 `agent/07.Python_AI_Agent系统架构设计.md` 与 `agent/08.Go_AI代理层设计.md`。
3. 只有需要对照 v1.0 单体式思路时，再看本文件。

## Boundary

- 本文件与 `archived/01.AI服务架构设计.md` 属于同一历史语义。
- 它不再承担当前 AI 架构 owner。
- 当前主线必须回到 `README.md` 和 `agent/`。
...
- 这是根层历史副本，不是当前架构入口。
- 现行 AI 方案已迁移到 `agent/` 主线。

## Skip Guide

- 只看当前方案：跳过本文件。
- 只做架构演进对照：看文档状态和架构设计前几节即可。
