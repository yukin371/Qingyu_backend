...
- current-bounded: Phase3 已完成专题中的大纲系统 Service/API 设计
...
- 本页属于 Phase3 完成态资料，不是当前大纲系统 owner。
- 其中 CRUD 设计与辅助函数应结合现行代码复核。
- 当前 AI 主线不由本页定义。
...
- 这页记录 Phase3 阶段里的大纲系统 Service/API 设计。
- 它不是 today 的唯一接口事实源。

## Skip Guide

- 只看当前主线：跳过本文件。
- 只看历史大纲服务设计：看接口定义和实现即可。
