...
- current-bounded: Phase3 已完成专题中的 AI 服务监控与可观测性设计

## Recommended Read Path

1. 先读 `README.md`。
2. 再读本页的指标、日志、追踪和告警设计。
3. 需要当前主线时，回 `../README.md` 与现行监控实现。

## Boundary

- 本页属于 Phase3 完成态资料，不是当前可观测性 owner。
- 指标命名、告警和仪表盘应结合 today 实现与运维配置确认。
- 当前 AI 主线不在本页定义。
...
- 这页记录 Phase3 阶段里 AI 服务如何做监控和可观测性。
- 它不是 today 运维事实的唯一来源。

## Skip Guide

- 只看当前主线：跳过本文件。
- 只看历史监控设计：看指标、追踪和告警三节即可。
