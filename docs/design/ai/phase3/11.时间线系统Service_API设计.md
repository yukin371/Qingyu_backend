...
- current-bounded: Phase3 已完成专题中的时间线系统 Service/API 设计

## Recommended Read Path

1. 先读 `README.md`。
2. 再读本页的 Service 接口与实现。
3. 需要当前主线时，回 `../README.md`。

## Boundary

- 本页是 Phase3 完成态资料，不是当前时间线能力 owner。
- 其中接口与索引设计应结合现行代码和 API 文档确认。
- 当前 AI 主线不在这里定义。
...
- 这页记录 Phase3 阶段里的时间线系统 Service/API 设计。
- 不应被当成 today 的唯一事实源。

## Skip Guide

- 只看当前主线：跳过本文件。
- 只看历史时间线服务设计：看接口定义和实现即可。
