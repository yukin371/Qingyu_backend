...
本文档负责性能目标、压测方式、优化策略与调优案例，是性能专项长文说明。

## Page Role

- 这里负责：性能目标、压测方法、数据库/缓存/网络/系统层优化。
- 不负责：本地启动、部署入门或监控面板使用说明。

## Recommended Read Path

1. [README_当前指南.md](./README_当前指南.md)
2. [监控体系使用指南.md](./监控体系使用指南.md)
3. [../deployment/服务启动指南.md](../deployment/服务启动指南.md)

## Boundary

- 本文中的 `8080` 压测 URL 默认按部署/样例环境理解。
- 若你在本地开发环境复现压测，请先确认实际启动端口；默认联调口径见 [../deployment/服务启动指南.md](../deployment/服务启动指南.md)。
