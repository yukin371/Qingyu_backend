...
本文档是部署长文手册，适合需要完整查看环境要求、部署步骤、Docker、生产部署和维护策略时阅读。

## Page Role

- 这里负责：部署全流程、运行检查、生产环境操作、维护升级。
- 不负责：本地日常热重载启动的最短路径。
...
- 本地开发联调默认使用 `air -c .air.toml`，端口口径以 [../deployment/服务启动指南.md](../deployment/服务启动指南.md) 的 `9090` 为准。
- 本文后续出现的 `8080` 示例，默认按部署/生产场景理解，不再代表本地开发默认端口。
