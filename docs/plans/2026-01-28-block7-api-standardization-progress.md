...
- **重构内容**:
  ...
  - 使用6位错误码 (0=成功, 100001=参数错误, 100601=未授权, 等)//废弃，改用4位错误码
  - 使用毫秒级时间戳 (`UnixMilli()`)
...
- **重构内容**:
  - 迁移所有响应调用从 `shared` 包到 `response` 包
  - 使用6位错误码 (0=成功, 100001=参数错误, 100202=冲突, 100601=未授权)//废弃，改用4位错误码
  - 使用毫秒级时间戳 (`UnixMilli()`)
...
type APIResponse struct {
    Code      int         `json:"code"`       // 0=成功, 6位错误码//废弃，改用4位错误码
