...
本文档负责“如何准备环境并跑完整集成测试”。如果你要搭本地联调环境、导入数据、启动后端、执行场景测试，优先看这一页。

## Page Role

- 这里负责：测试环境准备、测试数据导入、后端启动、场景级集成测试执行。
- 不负责：API 单元测试细节、gRPC 专项联调、测试规范本体。

## Recommended Read Path

1. [README_当前指南.md](./README_当前指南.md)
2. [本地测试数据初始化指南.md](./本地测试数据初始化指南.md)
3. [../deployment/服务启动指南.md](../deployment/服务启动指南.md)
...
- `configs/config.yaml` - 当前仓库可见的基础配置文件
- `config/README.md` - 配置加载、环境变量与目录结构说明
- 如需额外测试配置，请先按现有配置体系确认，不要直接沿用旧文档里的缺失路径
...
**解决方法**：
1. 确保服务器正在运行：`air -c .air.toml`
2. 检查本地联调端口 `9090` 是否可用
3. 检查防火墙设置
