...
### 1. 钱包 (`models/wallet/wallet.go`)

```go
type Wallet struct {
    ID        string    `bson:"_id,omitempty" json:"id"`
    UserID    string    `bson:"user_id" json:"userId"`
    Balance   float64   `bson:"balance" json:"balance"`              // 余额
    Frozen    bool      `bson:"frozen" json:"frozen"`                // 是否冻结
    FrozenAt  time.Time `bson:"frozen_at,omitempty" json:"frozenAt,omitempty"`
    CreatedAt time.Time `bson:"created_at" json:"createdAt"`
    UpdatedAt time.Time `bson:"updated_at" json:"updatedAt"`
}
```

### 2. 交易记录 (`models/wallet/wallet.go`)
...
type Transaction struct {
    ...
    Amount          float64   `bson:"amount" json:"amount"`            // 交易金额
    Balance         float64   `bson:"balance" json:"balance"`          // 交易后余额
    RelatedUserID   string    `bson:"related_user_id,omitempty" json:"relatedUserId,omitempty"` // 转账相关用户
    ...
    OrderNo         string    `bson:"order_no,omitempty" json:"orderNo,omitempty"`
    ThirdPartyNo    string    `bson:"third_party_no,omitempty" json:"thirdPartyNo,omitempty"`
    TransactionTime time.Time `bson:"transaction_time" json:"transactionTime"`
    CreatedAt       time.Time `bson:"created_at" json:"createdAt"`
...
### 3. 提现请求 (`models/wallet/wallet.go`)
...
type WithdrawRequest struct {
    ...
    AccountType   string    `bson:"account_type" json:"accountType"`   // alipay, wechat, bank
...
const (
    ...
    WithdrawStatusProcessed WithdrawStatus = "processed"  // 已处理
)
...
const (
    ...
    AccountTypeBank   AccountType = "bank"   // 银行卡
)
```
