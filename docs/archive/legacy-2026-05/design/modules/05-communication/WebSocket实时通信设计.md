...
**架构说明**：
- WebSocket 作为主应用的一个功能模块
- 连接管理在应用内存中维护
- 使用 Redis Pub/Sub 处理跨实例通信（多实例部署时）
- Gin 提供 WebSocket 升级和路由功能
...
- **消息队列**：Redis Pub/Sub（轻量级，跨实例通信）
- **连接存储**：应用内存 + Redis（多实例同步）
- **消息存储**：MongoDB
- **日志系统**：Zap（结构化日志）
- **监控工具**：Prometheus + Grafana

**未来升级方案**（微服务化时）：
- **消息队列**：Apache Kafka（大规模消息处理）
- **服务发现**：Consul / etcd
- **负载均衡**：独立的WebSocket网关
...
**阶段2：成长期**（并发连接 > 1000）
- 应用多实例部署 + Nginx 负载均衡
- Redis Pub/Sub 同步跨实例消息
- MongoDB 副本集

**阶段3：规模化**（并发连接 > 10000，可选）
- 考虑独立的 WebSocket 服务
- Kafka 消息队列
- 服务网格

**当前推荐**：Docker Compose 单机部署（支持1000+并发连接）
