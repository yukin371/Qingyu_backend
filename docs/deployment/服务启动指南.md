...
```batch
@echo off
cd /d "E:\Github\Qingyu\Qingyu_backend"
echo Starting backend service with air...
air -c .air.toml
pause
```
...
```batch
@echo off
cd /d "E:\Github\Qingyu\Qingyu_fronted"
...
- **Shared Auth**: http://localhost:9090/api/v1/shared/auth/
- **Shared Wallet**: http://localhost:9090/api/v1/shared/wallet/
- **Shared Storage**: http://localhost:9090/api/v1/shared/storage/
- **Shared Admin**: http://localhost:9090/api/v1/shared/admin/
...
查看 `Qingyu_fronted/.env` 文件或 `vite.config.ts`：
```javascript
...
- **后端**: 默认使用 `air -c .air.toml` 热重载启动；`air` 配置已指向 `cmd/server/main.go`
